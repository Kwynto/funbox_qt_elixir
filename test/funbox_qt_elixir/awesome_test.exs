defmodule FunboxQtElixir.AwesomeTest do
	...
	test "Parse description with links" do
		...
		result = FunboxQtElixir.Awesome.parse_description(description)
		assert result == {:description, " Begin description ", "http://localhost/", "somelink", " end description."}
	end
	...
	test "Parse data from GitHub API" do
		data = %{"status" => "loaded", "categories" => [], "resources" => [], 
		"allpacks" => [
			%{:name => "fsm", 
		...
		%{"status" => status, "categories" => _categories, "resources" => _resources, "allpacks" => allPacks} = result
	...
	test "Check for matches" do
		...
		assert result == [%{:title => "Algorithms and Data structures", :link => "algorithms-and-data-structures", :description => ""}]
