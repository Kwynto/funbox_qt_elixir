defmodule FunboxQtElixir.Awesome do
	...
	def runParse() do
		...
		%HTTPoison.Response{body: lines} = HTTPoison.get!("https://raw.githubusercontent.com/h4cc/awesome-elixir/master/README.md")
	...
	def parseAwesomeList(lines) do
		...
		[%EarmarkParser.Block.ListItem{blocks: [%EarmarkParser.Block.Text{} | categories]} | tableOfContent ] = tableOfContent
		[%EarmarkParser.Block.List{blocks: categories}] = categories
		categories = for %EarmarkParser.Block.ListItem{blocks: [%EarmarkParser.Block.Text{line: [name]}]} <- categories do
		...
		[%EarmarkParser.Block.ListItem{blocks: [%EarmarkParser.Block.Text{} | resources]} | _tableOfContent ] = tableOfContent
		[%EarmarkParser.Block.List{blocks: resources}] = resources
		resources = for %EarmarkParser.Block.ListItem{blocks: [%EarmarkParser.Block.Text{line: [name]}]} <- resources do
			{title, link} = parse_markdown_link(name)
			link = String.replace(link, "#", "")
			%{:title => title, :link => link, :description => ""}
		end
		...
		%{"status" => "loaded", "categories" => categories, "resources" => resources, "allpacks" => allPacks}
	end

	@description_regex ~r/^(.+)\[([^]]+)\]\(([^)]+)\)(.+)$/  # regex для описания пакета
	...
	def parse_description(description) do
		case Regex.run @description_regex, description do
			nil -> description # доп. ссылки нет -> вставляем описание как было
	...
	defp unblocking_content([head | tail], acc) do
		acc = unblocking_content(head, acc) # Перебераем пакеты в категории (обычная рекурсия = нарастание стека)
		unblocking_content(tail, acc) # Переходим к переборке остального тела (хвостовая рекурсия)
	end
	...
	defp iterate_content([
		...
		%EarmarkParser.Block.List{blocks: blocks, type: :ul} | tail], acc, categories) do
		# получаем описание категорий в процессе переборки основных болков
		cats = for category <- categories do
	...
	defp iterate_content([
		...
		%EarmarkParser.Block.Para{lines: _lines} | tail], acc, categories) do
	...
	defp check_list(list, heading) do
		for listItem <- list do
			line = try do
				%EarmarkParser.Block.ListItem{blocks: [%EarmarkParser.Block.Text{line: [line]}], type: :ul} = listItem
				line
			rescue
				_e -> %EarmarkParser.Block.ListItem{blocks: [%EarmarkParser.Block.Para{lines: [line]}], type: :ul} = listItem
						line
			end
			...
			%{:name => name, :link => link, :description => description, :heading => heading, :stars => 0, :lastupdate => 0}
		end
	end

	@line_regex ~r/^\[([^]]+)\]\(([^)]+)\) - (.+)([\.\!]+)$/ # regex для пакета
	...
	def parse_GitHub_data(data) do
		%{"status" => status, "categories" => categories, "resources" => resources, "allpacks" => allPacks} = data
		resultParse = parseStatus(allPacks, {:resState, status, []})
		{:resState, resStatus, resPacks} = resultParse
		# Формируем полный стейт
		%{"status" => resStatus, "categories" => categories, "resources" => resources, "allpacks" => resPacks}
	end
	...
	defp parseStatus([head | tail], acc) do
		try do
			...
			if oneStars == 0 and oneLU == 0 do
				oneLinkRepos = String.replace(oneLink, "https://github.com/", "https://fb-qt-elixir:Fmk9h0mda@api.github.com/repos/", global: false)
				%HTTPoison.Response{body: lines} = HTTPoison.get!(oneLinkRepos)
				{status, responseGHA} = Jason.decode(lines)
				acc2 = if status == :ok do
					{respUpdated, respStars, stat_active} = case responseGHA do
						...
						%{"documentation_url" => _doc_url_GH, "message" => _mes_GH, "url" => url_GH} ->
							oneLinkRepos = String.replace(url_GH, "https://api.github.com/", "https://fb-qt-elixir:Fmk9h0mda@api.github.com/", global: false)
							%HTTPoison.Response{body: lines} = HTTPoison.get!(oneLinkRepos)
							{_status, responseGHA} = Jason.decode(lines)
							%{"commits_url" => respCommitsUrl, "stargazers_count" => respStars} = responseGHA
							{getCommitUpdated(respCommitsUrl), respStars, 1}
						%{"documentation_url" => _doc_url_GH, "message" => _mes_GH} -> {0, 0, 0}
						_ -> {0, 0, 0}
					end
					acc2 = if stat_active != 0 do
						{:ok, respUpdatedDT, 0} = DateTime.from_iso8601(respUpdated)
						daysPassed = div((DateTime.to_unix(DateTime.now!("Etc/UTC")) - DateTime.to_unix(respUpdatedDT)), 86400)
	...
	defp getCommitUpdated(url) do
		try do
			oneLinkRepos = String.replace(url, "https://", "https://fb-qt-elixir:Fmk9h0mda@", global: false)
	...
	def check_for_matches(categories, allpacks) do
		listHeading = for onePack <- allpacks do
